...
- It serves as a demo showcasing "AI-assisted decision support".
...
- **Current state:** Sales manually compare engineer skill sheets against job requirements using spreadsheets or visual inspection.
  - **Problem 1 (Search precision):** Keyword-based exact matching cannot handle variations in expression (e.g., "Java engineer" vs. "Spring Framework experience"), causing missed matching opportunities.
...
### 4.3 Matching Reason Generation Feature
...
| Budget | OpenAI API (pay-as-you-go): ~$0.0001 for index build (one-time), ~$0.0004 per query (~$0.04 per 100 queries). Negligible cost at demo scale. (Rates as of 2026-05-10: text-embedding-3-small $0.02/1M tokens; gpt-4o-mini $0.15/$0.60 per 1M input/output tokens.) Streamlit Cloud free plan (sleep mode accepted). |
